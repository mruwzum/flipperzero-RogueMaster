...
FS_Error UFZ::Filesystem::filesystemInfo(
    const char* path,
    uint64_t* totalSpace,
    uint64_t* freeSpace) const noexcept {
...
void UFZ::Filesystem::getNextFilename(
    const char* dirname,
    const char* filename,
    const char* fileExtension,
    FuriString* nextFilename,
    const uint8_t maxLength) const noexcept {
...
bool UFZ::Directory::open(UFZ::File& f, const char* path) noexcept {
    ...
    if(file->file == nullptr) {
        if(file->storage == nullptr) return false;
        file->init();
    }
