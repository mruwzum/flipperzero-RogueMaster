...
External Flipper Zero app that shows Swatch Internet Time (@000–@999)
...
firmware fork. DCF77 is not received via SubGHz (CC1101 cannot tune 77.5 kHz).

## Requirements

- uFBT on the official release channel (SDK 1.4.3 / API 87.1)
- Flipper Zero with microSD card, USB data cable
- Optional DCF77 receiver module and ferrite antenna for live sync

## Controls

- Clock: Short OK or Back exits. Long OK opens settings.
- Settings: Up/Down select row; Left/Right change offset, Auto, or Invert;
  OK on Start sync opens DCF77 sync; Back saves and returns.
- DCF77 Sync: listens on GPIO C0; Back cancels; Long OK runs a demo fixture
  that writes a known time through the same RTC path (no RF needed).

Configure UTC offset in 15-minute steps so Biel Mean Time is derived correctly
from the Flipper RTC. Auto-sync (default On) starts sync once per app session
when the last success is older than 12 hours.

## DCF77 hardware

Wiring (confirm against your module silk):

- VDD → Flipper pin 9 (3V3)
- GND → Flipper pin 11 (GND)
- PON / P (active-low) → GND when required
- OUT / T → Flipper pin 16 (C0)

Keep the ferrite rod about 10 cm or more from the Flipper and near a window.
Reception across Europe is typically better at night.
...
Search for DCF77 module or 77.5 kHz RCC if a listing disappears. Prefer 1.1–3.3 V
boards with a tuned ferrite antenna.

## Note on firmware

Catalog builds target the official release SDK. Custom firmware may work when
API major/minor matches, but is not required.
