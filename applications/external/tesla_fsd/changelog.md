...
- **Removed the mis-derived "ssw0209 (byte0 hi)" variant profile.** It came from an earlier wrong reading of his capture; his real layout is `byte0` **low** nibble, which the standard parser already handles once HW4 is detected — so the profile was both incorrect and unnecessary. The auto-suggest matcher no longer offers it.

## 2.16-beta.19 — steer-jerk: inject at the engage edge (ESP32 Instant Engage / Minimal Inject fix)

- **ESP32: Instant Engage and Minimal Inject now actually fire at the engage edge (#108).** @dunckencn's TX-inclusive capture (thanks to beta.18's TX recording — and his own external logger) showed the injected `0x3EE` (bit46 ON) arriving **1.3–1.8 s after DAS 3**, i.e. 0.2–0.3 s *before* the abort — landing right on the `6 → 8` edge — and with Instant Engage *on*, the latency was inconsistent. Cause: the ESP32 copy of `fsd_ap_first_allows` had drifted from the shared Flipper one — it still gated at `das_ap_state >= 2` (the pre-beta.15 off-by-one) and **never checked `ap_first_edge` at all**, so Instant Engage did nothing on ESP32 and the 1 s stability debounce always applied. Combined with the ~1 s `0x3EE` broadcast cadence, the first inject landed anywhere up to ~2 s after engage, on the abort edge. Fixed: the ESP32 gate now matches the Flipper (engaged = `>= 3`), and **Instant Engage / Minimal Inject bypass the debounce** so the burst fires on the DAS-3 rising edge — ~1.5 s ahead of the abort window instead of on top of it. A burst meant for engage onset must never be delayed. Thanks @dunckencn for self-capturing TX and pinning the timing. **Please re-test Instant Engage + Minimal Inject on narrow roads** — this is the first build where they actually take effect on ESP32.
